...
class LineRefiner:
    ...
    def __init__(self, gap_ratio=0.02, align_ratio=0.01, debug=True):
    ...
    def refine(self, lines: List[Line], img=None) -> List[Line]:
        ...
        # ---- SCALE-AWARE PARAMS ----
        self.gap = int(self.gap_ratio * scale)
        self.align_tol = int(self.align_ratio * scale)
    ...
    def _show(self, img, lines):
        vis = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)

        for (x1, y1), (x2, y2) in lines:
            cv2.line(vis, (x1, y1), (x2, y2), (0, 255, 0), 2)

        cv2.imshow("Refined Lines", vis)
        cv2.waitKey(0)
